...
###
# Инициализация MongoDB с шардингом и репликацией
###
...
docker exec -it shard1_primary mongosh --port 27018 <<EOF
use somedb
print("Документов в первом шарде (Shard1):", db.helloDoc.countDocuments())
EOF

echo "Проверка данных в Shard2..."
docker exec -it shard2_primary mongosh --port 27017 <<EOF
use somedb
print("Документов во втором шарде (Shard2):", db.helloDoc.countDocuments())
EOF
